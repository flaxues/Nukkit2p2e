...
public enum Message {
    ...
    LGN_MISS_PWD("Password was not entered! Type /login <password>", 'c'),
    LGN_NEED_REG("You're not registered on server yet. You must register first.", 'c'),
    UNREG_MISS_PWD("Password was not entered! Type /unreg <password>", 'c'),
    ...
    public boolean tip(int seconds, CommandSender sender, Object... s) {
        ...
        if (player == null) sender.sendMessage(message);
            ...
            Server.getInstance().getScheduler().scheduleDelayedTask(new Runnable() {
                public void run() {
                    if (player.isOnline()) player.sendPopup(message);
                }
            }, 20 * i);
        return true;
    }
