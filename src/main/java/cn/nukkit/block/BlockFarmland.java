...
public class BlockFarmland extends BlockTransparent {
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_RANDOM) {
            ...
            if (this.level.getBlock(v.setComponents(x, this.y + 1, z).x, v.y, v.z) instanceof BlockCrops) {
                return 0;
            }
            ...
            for (int x = (int) this.x - 1; x <= this.x + 1; x++) {
                for (int z = (int) this.z - 1; z <= this.z + 1; z++) {
                    ...
                    Block block = this.level.getBlock(v.setComponents(x, this.y, z).x, v.y, v.z);
            ...
            Block block = this.level.getBlock(v.setComponents(x, y - 1, z).x, v.y, v.z);
