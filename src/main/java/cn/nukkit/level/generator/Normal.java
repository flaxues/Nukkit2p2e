...
public class Normal extends Generator {
    ...
    private final int seaHeight = 50;
    private final int seaFloorHeight = 38;
    private final int beathStartHeight = 48;
    private final int beathStopHeight = 55;
    private final int bedrockDepth = 1;
    private final int seaFloorGenerateRange = 1;
...
//        this.populators.add(dungeons);
    ...
    public void generateChunk(int chunkX, int chunkZ) {
        ...
        for (int genx = 0; genx < 16; genx++) {
            for (int genz = 0; genz < 16; genz++) {
                ...
                landHeightNoise = landHeightNoise - 0.5F;
                ...
                if (genyHeight < beathStartHeight) {
                    ...
                    if (canBaseGround) {
                    	int lhr = biome.isElevationDefined() ? biome.getMaxElevation() - biome.getMinElevation() : landHeightRange;
                        
                    	int baseGroundHeight = (int) (lhr * landHeightNoise) - lhr;
                    	if (baseGroundHeight < biome.getMinElevation())	{
                    		baseGroundHeight = baseGroundHeight + Math.abs(baseGroundHeight - biome.getMinElevation());
                    	}
    ...
    public void populateChunk(int chunkX, int chunkZ) {
        this.nukkitRandom.setSeed(0xabcdef ^ (chunkX << 8) ^ chunkZ ^ this.level.getSeed());
