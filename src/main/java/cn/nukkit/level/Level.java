...
public class Level implements ChunkManager, Metadatable {
    ...
    private final Map<Short, Object> changeBlocksFullMap = new HashMap<Short, Object>() {
		private static final long serialVersionUID = -7097745842420090526L;
    ...
    public boolean unload(boolean force) {
        ...
        for (Player player : new ArrayList<>(this.getPlayers().values())) {
            if (this == defaultLevel || defaultLevel == null) {
                ...
                player.teleport(this.server.getDefaultLevel().getSafeSpawn(player));
            }
        }
    ...
    public boolean save(boolean force) {
        ...
        this.provider.setRaining(false);
        this.provider.setRainTime(0);
        this.provider.setThundering(false);
        this.provider.setThunderTime(0);
    ...
    public int getFullLight(int x, int y, int z) {
        FullChunk chunk = this.getChunk(x >> 4, z >> 4, false);
        int level = 0;
        if (chunk != null) {
            level = chunk.getBlockSkyLight(x & 0x0f, y & 0xff, z & 0x0f);
            // TODO: decrease light level by time of day
            if (level < 15) {
                level = Math.max(chunk.getBlockLight(x & 0x0f, y & 0xff, z & 0x0f), level);
                // todo: check this
            }
        }
    ...
    public Block getBlock(int x, int y, int z, boolean cached) {
        long chunkIndex = Level.chunkHash(x >> 4, z >> 4);
        ...
        if (cached && (block = (Block) this.blockCache.get(index)) != null) {
            return block;
        } else if (y >= 0 && y < 128 && (chunk = this.chunks.get(chunkIndex)) != null) {
            fullState = chunk.getFullBlock((int) x & 0x0f, (int) y & 0x7f,
                    (int) z & 0x0f);
        } else {
            fullState = 0;
        }
    ...
    public Position getSafeSpawn(boolean valueToMakeSureThisIsCalled)	{
    	Position pos = new Position(1000, 128, 1000);
    ...
    public Position getSafeSpawn(Player player)	{
    	Position pos = this.getSpawnLocation();
		
		if (player.getPosition() != null)	{
			pos = player.getPosition();
		}/*
		
		if (-128 <= player.spawnPosition.x && player.spawnPosition.x <= 128) {
			if (-128 <= player.spawnPosition.z && player.spawnPosition.z <= 128) {
				...
				// spawn
				Random r = new Random(); // Don't want to affect nukkitRandom
				pos.x = r.nextInt(257) - 128;
				pos.z = r.nextInt(257) - 128;
				pos.y = r.nextInt(127) + 1;
				Level world = this.server.getLevelByName("world");

				while (!(world.getBlock(pos).getId() == Block.AIR
						&& world.getBlock(pos.setZ(pos.z + 1)).getId() == Block.AIR
						&& world.getBlock(pos.setZ(pos.z - 2)).getId() != Block.LAVA
						&& world.getBlock(pos).getId() != Block.STILL_LAVA
						&& world.getBlock(pos).getId() != Block.WATER
						&& world.getBlock(pos).getId() != Block.STILL_WATER
						&& world.getBlock(pos).getId() != Block.FIRE
						&& world.getBlock(pos).getId() != Block.AIR)) {
					pos.x = r.nextInt(257) - 128;
					pos.z = r.nextInt(257) - 128;
					pos.y = r.nextInt(127) + 1;
				}
