package mobs.milk.pureentities.entity.projectile;
    ...
    public boolean onUpdate(int currentTick){
        ...
        if(!this.hadCollision && this.critical){
            this.level.addParticle(new CriticalParticle(this.add(
                this.getWidth() / 2 + Utils.rand(-100, 100) / 500,
                this.getHeight() / 2 + Utils.rand(-100, 100) / 500,
                this.getWidth() / 2 + Utils.rand(-100, 100) / 500)));
        ...
        if(this.age > 1200 || this.isCollided){
            if(this.isCollided && this.canExplode){
                Explosion explosion = new Explosion(this, 2.8f, this.shootingEntity);
                explosion.explodeA();
                explosion.explodeB();
                explosion.fire();
            }
